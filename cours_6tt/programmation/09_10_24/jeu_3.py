...
class Jeu(object):
    ...
    def jeu_depart (self):
        ...
        self.bouton_jouer = Button(self.fenetre_dessous,text='jouer',command=self.jeu_cartes_bataille)# self.jeu_cartes_bataille
    ...
    # Jeu Action
    def jeu_cartes_bataille(self):
        while len(self.cartes_objet.paquet_j1)+len(self.cartes_objet.paquet_j2) != 0 :
            self.paquet_milieu.append(self.cartes_objet.paquet_j1.pop(0))
            self.paquet_milieu.append(self.cartes_objet.paquet_j2.pop(0))
            
            if self.cartes_objet.valeurs.index(self.paquet_milieu[0].split(" ")[0]) > self.cartes_objet.valeurs.index(self.paquet_milieu[1].split(" ")[0]):
                self.carte_joueur_jeu = PhotoImage(file ="./cartes/" + self.paquet_milieu[0].split (" ")[0].lower () + "_" + self.paquet_milieu[0].split (" ")[2].lower () + ".png")
                self.carte_joueur.config (image=self.carte_joueur_jeu)
                self.carte_ordinateur_jeu = PhotoImage(file ="./cartes/" + self.paquet_milieu[1].split (" ")[0].lower () + "_" + self.paquet_milieu[1].split (" ")[2].lower () + ".png")
                self.carte_ordinateur.config (image=self.carte_ordinateur_jeu)
            elif self.cartes_objet.valeurs.index(self.paquet_milieu[0].split(" ")[0]) < self.cartes_objet.valeurs.index(self.paquet_milieu[1].split(" ")[0]):
                self.carte_joueur_jeu = PhotoImage(file ="./cartes/" + self.paquet_milieu[0].split (" ")[0].lower () + "_" + self.paquet_milieu[0].split (" ")[2].lower () + ".png")
                self.carte_joueur.config (image=self.carte_joueur_jeu)
                self.carte_ordinateur_jeu = PhotoImage(file ="./cartes/" + self.paquet_milieu[1].split (" ")[0].lower () + "_" + self.paquet_milieu[1].split (" ")[2].lower () + ".png")
                self.carte_ordinateur.config (image=self.carte_ordinateur_jeu)
